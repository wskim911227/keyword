...
function toFriendlyEmailError(error) {
  ...
  if (message.includes("Application-specific password required")) {
    return (
      "Gmail 앱 비밀번호가 필요합니다. 일반 비밀번호가 아니라 Google 앱 비밀번호를 " +
      "Vercel의 GMAIL_APP_PASSWORD에 설정해 주세요. (2단계 인증 필요)"
    );
  }
  if (message.includes("Invalid login") || message.includes("Username and Password not accepted")) {
    return (
      "Gmail 로그인에 실패했습니다. GMAIL_USER와 GMAIL_APP_PASSWORD가 올바른지 확인해 주세요."
    );
  }
  return message || "이메일 발송 중 오류가 발생했습니다.";
}
