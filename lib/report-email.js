...
function normalizeFromAddress(raw) {
  ...
  const simpleEmail = value.match(/^([^<>\s]+@[^<>\s]+\.[^<>\s]+)$/);
  if (simpleEmail) {
    return simpleEmail[1];
  }

  const namedEmail = value.match(/^(.+?)\s*<([^<>@\s]+@[^<>@\s]+\.[^<>@\s]+)>$/);
  if (namedEmail) {
    const name = namedEmail[1].trim().replace(/^["']|["']$/g, "");
    const email = namedEmail[2].trim();
...
async function sendReportEmail({ title, report }) {
  ...
  const from = normalizeFromAddress(process.env.RESEND_FROM || DEFAULT_FROM);
...
function toFriendlyEmailError(error) {
  ...
  if (message.includes("Invalid `from` field") || message.includes("Invalid from field")) {
    return (
      "RESEND_FROM 형식이 올바르지 않습니다. " +
      "예: onboarding@resend.dev 또는 Issue Report <onboarding@resend.dev>"
    );
  }
  if (message.includes("domain") || message.includes("verified")) {
    return (
      "발신 도메인이 인증되지 않았습니다. Resend에서 도메인을 인증하거나 " +
      "테스트용 RESEND_FROM=onboarding@resend.dev 를 사용해 주세요."
    );
  }
